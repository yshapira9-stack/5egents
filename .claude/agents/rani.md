...
**מבנה הפלט — תיקיה לכל תכשיט, מספר גלובלי עולה:** כל פריט מקבל תיקיה משלו
שמכילה את *כל* הפרטים שלו (9 התמונות + SEO + תמונת המקור). המספר הסידורי
**גלובלי** (רץ על כל סוגי התכשיטים יחד), ושם התיקיה הוא `<NNN>-<סוג>-<תיאור-קצר>`.

קבע את המספר הבא (סרוק תיקיות קיימות בתבנית `NNN-*`, קח מקסימום + 1) וצור תיקיה:
```bash
ROOT="rani/outputs/עיצובי תכשיטים"
NEXT=$(printf "%03d" $(( $(find "$ROOT" -maxdepth 1 -mindepth 1 -type d -printf "%f\n" \
  | grep -oE '^[0-9]+' | sort -n | tail -1 | sed 's/^0*//' ) + 1 )))
DIR="$ROOT/${NEXT}-<סוג>-<תיאור-קצר>"   # למשל 005-טבעת-טורקיז
mkdir -p "$DIR"
```
...
```bash
# דוגמה: גרסת סטודיו → אתר (שם קצר בתוך תיקיית הפריט)
node rani/scripts/process.mjs "$DIR/_base-studio.png" "$DIR/סטודיו-אתר.png" 1200 800
```

**על הלוגו והכיתוב:**
- הלוגו: `rani/assets/לוגו-סמל.png` — **הריבוע (סמל ה-Y) בלבד, ללא הכיתוב**.
  (הלוגו המלא המקורי שמור ב-`rani/assets/לוגו יניב המעצב.png` וב-`Output/images/logo.png`.)
  אם הסמל חסר — הסקריפט מדלג ומתעד אזהרה ב-stdout (`"logo": false`), לא קורס.
...
### שלב 5 — מבנה הקבצים בתוך תיקיית הפריט

התיקיה כבר ייחודית לפריט, לכן שמות הקבצים **קצרים** — `<סגנון>-<פלטפורמה>.png`:
```
<NNN>-<סוג>-<תיאור>/
├── מקור.<סיומת>          ← תמונת המקור (העתק את קובץ הקלט לכאן)
├── סטודיו-אתר.png        סטודיו-פייסבוק.png        סטודיו-אינסטגרם.png
├── דוגמן-אתר.png         דוגמן-פייסבוק.png         דוגמן-אינסטגרם.png
├── אווירה-אתר.png        אווירה-פייסבוק.png        אווירה-אינסטגרם.png
└── SEO.txt
```

שמור גם את **תמונת המקור** בתיקיה (`cp "<source_path>" "$DIR/מקור.<סיומת>"`).
...
**עמידות — עבד כל בסיס מיד:** במערכת הזו נצפו קבצי `_base-*.png` שנעלמים בין
קריאות Bash נפרדות (כנראה סנכרון/אנטי-וירוס). לכן **בכל סגנון, צור את הבסיס
ועבד אותו ל-3 הפלטפורמות באותה קריאת Bash** — אל תסתמך על קבצי בסיס ששורדים
בין פקודות.
